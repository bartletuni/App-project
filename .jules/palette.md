...
## 2024-05-14 - Semantic Navigation Elements
**Learning:** Using `<div onClick={() => router.push(...)}>` for navigation breaks keyboard accessibility (no native focus, Tab support, or Enter key activation) and screen reader predictability.
**Action:** Always use native semantic elements like Next.js `<Link>` or `<a>` tags for routing to ensure inherent accessibility and progressive enhancement.
