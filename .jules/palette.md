...
## 2026-06-11 - High-performance background animations\n**Learning:** The project uses framer-motion for general UI animations. However, for high-performance visual effects like full-screen background particles, a custom HTML `<canvas>` paired with a `requestAnimationFrame` loop provides significantly better performance. When implementing such effects, tracking the mouse coordinates via a `useRef` instead of React state (`useState`) is critical to avoid unnecessary and expensive re-renders of the component tree on every single `mousemove` event.\n**Action:** Use `<canvas>` for dense particle systems and decouple rapidly changing event inputs (like mouse movements) from React's rendering cycle using `useRef`.
