  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-transparent flex flex-col items-center justify-center gap-4">
        ...
        <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-cream-500">Loading material index…</p>
      </div>
    );
  }
                ...
                <Panel className="group h-full flex flex-col rounded-md hover:border-clay-500/35 transition-colors">
                  {m.imageId && (
                    <div className="h-40 w-full overflow-hidden border-b border-clay-500/15">
                      ...
                      <img src={`/api/download/${m.imageId}`} alt={m.name} className="h-full w-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition duration-500" />
                    </div>
                  )}
                  <div className="flex-1 flex flex-col p-6">
                    ...
                    <h3 className="font-display text-2xl text-cream-100 mb-2">{m.name}</h3>
