...
function RequestFormContent({ onFormSubmit }: { onFormSubmit: () => void }) {
  ...
  return (
    <div className="bg-espresso-800/72 backdrop-blur-md p-6 sm:p-8 rounded-2xl shadow-sm border border-clay-500/18">
      <h2 className="text-xl font-bold mb-6 text-cream-200 border-b border-espresso-700 pb-4">Create New Request</h2>

      <div className="bg-amber-500/15 border border-amber-500/30 text-amber-200 p-4 mb-6 rounded-xl flex gap-3 items-start" role="alert">
        ...
        <div>
          ...
          <p className="text-sm mt-1 text-amber-300">Orders can only be cancelled within <strong>30 minutes</strong> of placing the request.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm hover:shadow-glow text-sm font-bold text-white bg-clay-600 hover:bg-clay-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-clay-500 disabled:opacity-70 disabled:cursor-not-allowed transition-all active:scale-[0.98] mt-6"
        >
          {loading ? (
...
export default function RequestForm({ onFormSubmit }: { onFormSubmit: () => void }) {
  return (
    <Suspense fallback={<div className="bg-espresso-800/45 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-sm border border-clay-500/12 animate-pulse h-96"></div>}>
